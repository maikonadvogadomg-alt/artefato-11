...
export default function GitHubModal({
  ...
  visible: boolean;
  onClose: () => void;
  onPublishComplete?: (pagesUrl: string) => void;
}) {
  ...
  // Reset when opened + auto-detecta token do clipboard
  useEffect(() => {
    if (visible) {
      ...
      setPushResult(null);
      setRepoSearch("");
      if (t) { fetchUser(t); return; }
      // Sem token salvo: tenta ler clipboard e preencher automaticamente
              ...
              <View style={{ gap: 8, marginBottom: 8 }}>
                ...
                {pagesInfo && (
                  <View style={{ borderRadius: 10, padding: 14, backgroundColor: "#22c55e14", borderWidth: 1, borderColor: "#22c55e44", gap: 8 }}>
                    ...
                    <TouchableOpacity
                      onPress={() => { onPublishComplete?.(pagesInfo.url); onClose(); }}
                      style={{ backgroundColor: "#f59e0b22", borderRadius: 8, padding: 10, flexDirection: "row", alignItems: "center", gap: 8, borderWidth: 1, borderColor: "#f59e0b55" }}
                    >
                      <Feather name="package" size={14} color="#f59e0b" />
                      <Text style={{ color: "#f59e0b", fontWeight: "700", fontSize: 13, flex: 1 }}>📦 Converter em APK</Text>
