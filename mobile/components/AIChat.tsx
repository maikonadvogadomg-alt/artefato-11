...
export default function AIChat({ pendingMessage, onPendingMessageConsumed }: AIChatProps = {}) {
  ...
  const sendMessage = useCallback(async (overrideText?: string) => {
    ...
    // Arquivo de contexto: o que o usuário selecionou na árvore do chat, senão o do editor
    const contextFile = (chatFileId && activeProject?.files.find(f => f.id === chatFileId)) || activeFile;
...
5. CONTEXTO DO PROJETO: Você tem acesso a TODOS os arquivos do projeto. Quando pedido para criar ou editar um arquivo, gere o código completo em bloco \`\`\`linguagem para que o usuário possa aplicar com o botão "Aplicar".
...
7. CRIAR/EDITAR ARQUIVOS: Sempre gere o conteúdo COMPLETO do arquivo em bloco de código. O usuário usará o botão "Aplicar" para salvar no projeto. Informe o nome do arquivo que deve receber o código.
...
8. PLANO DE EXECUÇÃO: Para tarefas grandes, liste os passos numerados e peça confirmação para começar. Depois execute passo a passo dando feedback de cada um.`;
      ...
      if (contextFile) {
        systemContext += `\n\n📄 ARQUIVO EM FOCO: ${contextFile.name} (${contextFile.language})\n\`\`\`${contextFile.language}\n${contextFile.content.slice(0, 3000)}\n\`\`\``;
      }
      if (activeProject) {
        systemContext += `\n\n📁 PROJETO: ${activeProject.name} (${activeProject.files.length} arquivo${activeProject.files.length !== 1 ? "s" : ""})`;
        const outrosArquivos = activeProject.files.filter(f => f.id !== contextFile?.id);
  ...
  return (
    ...
    >
      ...
      {/* Seletor de arquivo — árvore compacta do projeto */}
      {chatMode === "dev" && activeProject && activeProject.files.length > 0 && (
        <View style={{ borderBottomWidth: 1, borderBottomColor: colors.border }}>
          <TouchableOpacity
            onPress={() => { setShowFileList(v => !v); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }}
            style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 10, paddingVertical: 5, gap: 6 }}
          >
            <Feather name="folder" size={11} color={colors.primary} />
            <Text style={{ color: colors.primary, fontSize: 11, fontWeight: "700", flex: 1 }} numberOfLines={1}>
              {(() => { const cf = (chatFileId && activeProject.files.find(f => f.id === chatFileId)) || activeFile; return cf ? cf.name : "Toque para escolher arquivo"; })()}
            </Text>
            <Text style={{ color: colors.mutedForeground, fontSize: 10 }}>{activeProject.files.length} arq</Text>
            <Feather name={showFileList ? "chevron-up" : "chevron-down"} size={11} color={colors.mutedForeground} />
          </TouchableOpacity>
          {showFileList && (
            <ScrollView style={{ maxHeight: 130 }} contentContainerStyle={{ paddingHorizontal: 6, paddingBottom: 6, gap: 1 }}>
              {activeProject.files.map(f => {
                const selected = f.id === (chatFileId || activeFile?.id);
                return (
                  <TouchableOpacity
                    key={f.id}
                    onPress={() => { setChatFileId(f.id); setShowFileList(false); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }}
                    style={{ flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 8, paddingVertical: 6, borderRadius: 6, backgroundColor: selected ? colors.primary + "1a" : "transparent" }}
                  >
                    <Feather name="file-text" size={12} color={selected ? colors.primary : colors.mutedForeground} />
                    <Text style={{ color: selected ? colors.primary : colors.foreground, fontSize: 12, flex: 1 }} numberOfLines={1}>{f.name}</Text>
      ...
      {/* Atalhos compactos (modo Dev) */}
      {chatMode === "dev" && activeProject && !isLoading && (
        <ScrollView
          horizontal
          showsHorizontalScrollIndicator={false}
          contentContainerStyle={{ paddingHorizontal: 8, paddingVertical: 4, gap: 4 }}
          style={[{ borderBottomWidth: 1, borderBottomColor: colors.border }]}
        >
          {[
            { label: "📊 SPEC", msg: `Gere uma ESPECIFICAÇÃO COMPLETA do projeto "${activeProject.name}" (arquivos: ${activeProject.files.map(f => f.name).join(", ") || "nenhum"}). Inclua: o que o app faz, rotas de API, funções principais, o que funciona, limites, próximos passos. Use markdown com títulos e tabelas.` },
            { label: "📋 Plano", msg: `Analise o projeto "${activeProject.name}" e gere um PLANO.md completo com: objetivo, stack, estrutura, funcionalidades e próximos passos. Use markdown.` },
            { label: "⚙️ Docs", msg: `Documente o projeto "${activeProject.name}" em um SISTEMA.md técnico: descrição, variáveis de ambiente, endpoints da API, dependências e notas de arquitetura.` },
            { label: "🐛 Review", msg: `Revise todos os arquivos do projeto "${activeProject.name}" e liste bugs, problemas de performance e sugestões de melhoria.` },
            { label: "📦 Deps", msg: `Quais bibliotecas instalar para o projeto "${activeProject.name}"? Liste com nome, versão e comando npm install.` },
            { label: "🔌 API", msg: `Liste todas as rotas de API do projeto "${activeProject.name}": método, caminho, parâmetros e retorno. Tabela markdown.` },
            { label: "✨ Criar", msg: `Preciso criar um novo arquivo no projeto "${activeProject.name}". Pergunte o nome e conteúdo, depois gere o arquivo completo.` },
          ].map((item) => (
            <TouchableOpacity
              key={item.label}
              onPress={() => sendMessage(item.msg)}
              style={[styles.quickReplyChip, { backgroundColor: colors.card, borderColor: colors.primary + "33", paddingHorizontal: 9, paddingVertical: 4 }]}
            >
              <Text style={[styles.quickReplyText, { color: colors.primary, fontSize: 11 }]}>{item.label}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      )}
      ...
      {/* Chips de sugestão compactos — aparecem ACIMA do input quando não há mensagens */}
          ...
          >
            {SUGGESTION_CHIPS.map((chip) => (
              <TouchableOpacity
                ...
                style={[styles.suggestionChip, { backgroundColor: colors.card, borderColor: colors.border, paddingHorizontal: 9, paddingVertical: 5, borderRadius: 14 }]}
              >
                <Text style={[styles.suggestionChipText, { color: colors.foreground, fontSize: 11 }]}>{chip.label}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
