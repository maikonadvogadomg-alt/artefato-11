...
export default function Terminal({ runCmd, onCmdRan }: TerminalProps = {}) {
  ...
  const [serverOnline, setServerOnline] = useState<boolean | null>(() => TERMINAL_API ? null : false);
    ...
    (code: string): string => {
      ...
      try {
        ...
        try {
          ...
          // new Function() é bloqueado em alguns builds Hermes — tenta eval() como fallback
          try {
            const wrapped = `(function(console,Math,JSON,Date,Array,Object,String,Number,Boolean,RegExp,Error,Promise,setTimeout,clearTimeout){"use strict";return(function(){\n${code}\n})();})`;
            // eslint-disable-next-line no-eval
            fn = eval(wrapped) as (...args: unknown[]) => unknown;
          } catch (evalErr2) {
            const ms = Date.now() - start;
            return `❌ Execução de código não disponível neste build.\nDica: use Expo Go (dev) para rodar js>.\nErro: ${evalErr2 instanceof Error ? evalErr2.message : String(evalErr2)}\n⚡ ${ms}ms`;
          }
        }
        const result = fn(
        ...
        if (result !== undefined && result !== null) {
          try {
            ...
            if (result && typeof (result as Promise<unknown>).then === "function") {
              (result as Promise<unknown>).then((v: unknown) => {
    ...
    >
      ...
      <View style={[styles.tabBar, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 4, marginRight: 4 }}>
          <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: serverBusy ? "#f59e0b" : serverOnline ? "#00d4aa" : "#8b949e" }} />
          <Text style={{ fontSize: 9, color: serverBusy ? "#f59e0b" : serverOnline ? "#00d4aa" : "#8b949e", fontWeight: "700" }}>
            {serverBusy ? "EXEC" : serverOnline ? "LIVE" : "LOCAL"}
          </Text>
        </View>
      </View>
